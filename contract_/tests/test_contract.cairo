...
fn deploy_big_inc_genesis(
    usdt_address: ContractAddress, usdc_address: ContractAddress, owner: ContractAddress,
) -> ContractAddress {
...
fn test_seize_shares_success() {
    ...
    assert(
        big_inc.get_shares(owner) == owner_shares_before + user1_shares,
        'Owner should have seized shares',
    );
    assert(!big_inc.is_shareholder(user1), 'User1 should not be shareholder');
}
